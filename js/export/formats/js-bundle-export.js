...
export const ZOLTO_RUNTIME_JS = `
function initZoltoInteractivity(root) {
  if (!root) return;
  ...
  // 2. Interactive Tabs
  const tabContainers = root.querySelectorAll('.zl-tabs');
  tabContainers.forEach(tabs => {
    const tabBtns = tabs.querySelectorAll('.zl-tab-btn');
    const tabPanels = tabs.querySelectorAll('.zl-tab-panel');
  ...
  // 3. Quiz Engine
  const quizzes = root.querySelectorAll('.zl-quiz');
  quizzes.forEach(quiz => {
    const options = quiz.querySelectorAll('.zl-quiz-opt');
    const submitBtn = quiz.querySelector('.zl-quiz-submit, [data-quiz-submit], .zl-quiz-btn');
    const feedback = quiz.querySelector('.zl-quiz-feedback');
    ...
    if (submitBtn) {
      submitBtn.addEventListener('click', () => {
        if (!selectedOpt) {
          if (feedback) {
            feedback.innerHTML = '<span style="color:var(--zl-accent-amber);">Please select an option first.</span>';
            feedback.style.display = 'block';
          }
          return;
        }
        const isCorrect = selectedOpt.getAttribute('data-correct') === 'true' || selectedOpt.dataset.correct === '1' || selectedOpt.classList.contains('correct');
        options.forEach(o => {
          if (o.getAttribute('data-correct') === 'true' || o.dataset.correct === '1' || o.classList.contains('correct')) {
            o.classList.add('zl-opt-correct');
          } else if (o === selectedOpt && !isCorrect) {
            o.classList.add('zl-opt-incorrect');
          }
        });
        if (feedback) {
          feedback.style.display = 'block';
          if (isCorrect) {
            feedback.innerHTML = '<span style="color:var(--zl-accent-green);">✓ Correct! Outstanding work.</span>';
          } else {
            feedback.innerHTML = '<span style="color:var(--zl-accent-red);">✗ Incorrect. Check the solution.</span>';
          }
        }
      });
    }
  ...
  // 4. Flashcard Deck Engine
  const flashcardDecks = root.querySelectorAll('.zl-flashcard-deck, .zl-flashcards');
  flashcardDecks.forEach(deck => {
    const cards = deck.querySelectorAll('.zl-flashcard');
    let currentIdx = 0;
    const prevBtn = deck.querySelector('.zl-fc-prev');
    const nextBtn = deck.querySelector('.zl-fc-next');
    const counter = deck.querySelector('.zl-fc-counter');
